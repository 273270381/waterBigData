        ...
        <el-col :span="6">
          <div class="table-container">
            <el-table ref="orderTable"
              ...
              <el-table-column label="雨量(mm)" align="center">
                <template slot-scope="scope">{{scope.row.value}}</template>
              </el-table-column>
              <el-table-column label="累计雨量(m)" align="center">
                <template slot-scope="scope">{{scope.row.cumulativeRain}}</template>
              </el-table-column>
            </el-table>
          </div>
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        chartSettings: {
          ...
          area:true,
          axisSite: { right: ['cumulativeRain']},
          labelMap: {'value': '雨量','cumulativeRain' : '累积雨量'}},
        loading: false,
        dataEmpty: true,
    ...
    methods: {
      ...
      getData(){
        setTimeout(() => {
          this.chartData = {
            columns: ['captureTime', 'value','cumulativeRain'],
            rows: []
          };
      ...
      getList() {
        ...
        fetchList(this.listQuery).then(response => {
          ...
          for(let i=0;i<this.list.length;i++){
            ...
            this.list[i].cumulativeRain = this.cumulativeValue + this.list[i].value;
            this.cumulativeValue = this.list[i].value + this.cumulativeValue;
          }
